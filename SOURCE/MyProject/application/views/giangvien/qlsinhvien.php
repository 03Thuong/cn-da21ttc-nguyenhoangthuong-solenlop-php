<div class="main-content">
    <di class="content">
        ...
        <form action="<?= base_url('sinhvien/import_excel'); ?>" method="post" enctype="multipart/form-data"
            class="form-import">
            <!-- Dropdown chọn mã nhóm -->
            <div class="form-group-import">
                <label for="ma_nhom">Chọn nhóm:</label>
                <select name="ma_nhom" id="ma_nhom" required class="select-import">
                    <option value="">-- Chọn nhóm môn học--</option>
                    <?php foreach ($nhommonhoc as $nhom): ?>
                        <option value="<?= htmlspecialchars($nhom['manhom'] . '-' . $nhom['maMH']); ?>">
                            <?= htmlspecialchars($nhom['manhom'] . ' - ' . $nhom['maMH'] . ' - ' . $nhom['TenMH']); ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <form method="get" action="<?= base_url('search_sv_gv'); ?>" id="filterForm" style="margin-left:30px;">
        ...
        <div class="search-kh" style="margin-bottom:-20px; margin-left: 270px;">
...
<nav aria-label="Page navigation">
    <ul class="pagination">
        ...
        <li class="page-item <?php echo ($current_page == 1) ? 'disabled' : ''; ?>">
            <a class="page-link" href="<?php echo base_url('phantrang_sv_gv?page=' . ($current_page - 1) . (isset($keyword) ? '&keyword=' . urlencode($keyword) : '') . (isset($ma_nhom) ? '&ma_nhom=' . urlencode($ma_nhom) : '') . (isset($ma_mon) ? '&ma_mon=' . urlencode($ma_mon) : '') . (isset($to_th) ? '&to_th=' . urlencode($to_th) : '')); ?>">
                <<</a>
        </li>
        ...
        for ($i = $start; $i <= $end; $i++): ?>
            <li class="page-item <?php echo ($i == $current_page) ? 'active' : ''; ?>">
                <a class="page-link" href="<?php echo base_url('phantrang_sv_gv?page=' . $i . (isset($keyword) ? '&keyword=' . urlencode($keyword) : '') . (isset($ma_nhom) ? '&ma_nhom=' . urlencode($ma_nhom) : '') . (isset($ma_mon) ? '&ma_mon=' . urlencode($ma_mon) : '') . (isset($to_th) ? '&to_th=' . urlencode($to_th) : '')); ?>">
                    <?php echo $i; ?>
                </a>
            </li>
        <?php endfor; ?>
        ...
        <li class="page-item <?php echo ($current_page == $total_pages) ? 'disabled' : ''; ?>">
            <a class="page-link" href="<?php echo base_url('phantrang_sv_gv?page=' . ($current_page + 1) . (isset($keyword) ? '&keyword=' . urlencode($keyword) : '') . (isset($ma_nhom) ? '&ma_nhom=' . urlencode($ma_nhom) : '') . (isset($ma_mon) ? '&ma_mon=' . urlencode($ma_mon) : '') . (isset($to_th) ? '&to_th=' . urlencode($to_th) : '')); ?>">>></a>
        </li>
    </ul>
</nav>
